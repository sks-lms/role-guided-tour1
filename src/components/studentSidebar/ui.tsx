                    ...
                    {!state.collapsed && (
                        <div className="animate-fade-in">
                            <div className="flex items-center gap-3 p-3 rounded-lg bg-sidebar-accent">
                                <div className="w-8 h-8 bg-gradient-secondary rounded-full flex items-center justify-center" onClick={() => {
                                    dispatch(authActions.logout());
                                    handlers.navigate("/");
                                }}>
                                    {/* <User className="w-4 h-4 text-white" /> */}
                                    <img
                                        src={state.profileData?.avatarUrl ?? 'https://plus.unsplash.com/premium_photo-1690407617542-2f210cf20d7e?q=80&w=687&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'}
                                        alt={state.profileData?.name ?? 'Ava Johnson'}
